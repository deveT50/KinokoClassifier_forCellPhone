	...
	for epoch in range(args.epoch):
		...
		for idx in xrange(len(train_list)):
			...
			if i == args.batchsize or idx==len(train_list):
				...
				i = 0
				#print "!---train---!"
				#loss, accuracy=perform(x_batch,y_batch,train,len(train_list),cnt)
...
#10回ごとにvalidation用バッチを作る
		if epoch % 10 == 0 and epoch!=0 or epoch==args.epoch-1: #each 10 count, validation batch also be prepared
			val_acc_mean=0
			j = 0
			#cnt=0
